...
func (b *BTOrdIdx) writeMrc721(txn *badger.Txn, block *HookBlock, inscr *HookInscription, mrc721Data *MRC721Protocol) (err error) {
	...
	if err != nil {
		if err == badger.ErrKeyNotFound {
			...
			// deploy message
			jsonMessage := fmt.Sprintf(`{"name":"%s","tick":"%s","hash":"%s"}`, mrc721Data.Miner.GetUpperName(), mrc721Data.Token.Tick, block.BlockHash)
			b.RecorderPush(inscr.Address, "deploy", jsonMessage)
...
func (b *BTOrdIdx) addNewMrc721(txn *badger.Txn, block *HookBlock, inscr *HookInscription, mrc721Data *MRC721Protocol, mrc721Count int) (err error) {
	...
	// mint message
	jsonMessage := fmt.Sprintf(`{"name":"%s","tick":"%s","hash":"%s"}`, mrc721Data.Miner.GetUpperName(), mrc721Data.Token.Tick, block.BlockHash)
	b.RecorderPush(inscr.Address, "mint", jsonMessage)
	...
	for _, transferItem := range block.Transfers {
		...
		// --- transfer 721 ---
		{
			...
			if oldAddr == "" {
				...
				fromAddress := hookInscription.Address
				...
				// transfer message mrc721
				jsonMessage := fmt.Sprintf(`{ "inscriptionID":"%s","toAddress":"%s", "hash":"%s","amount":"0"}`, hookInscription.ID, toAddress, block.BlockHash)
				b.RecorderPush(fromAddress, "transfer", jsonMessage)
		...
		// --- transfer 20 ---
		{
			...
			if oldAddr == "" {
				...
				// transfer message mrc20
				jsonMessage := fmt.Sprintf(`{ "inscriptionID":"%s","toAddress":"%s",,"amount":"%s", "hash":"%s"}`, transferItem.ID, toAddress, transferAmount.String(), block.BlockHash)
				b.RecorderPush(mrc20Inscription.Address, "transfer", jsonMessage)
...
func (b *BTOrdIdx) writeMrc20(txn *badger.Txn, block *HookBlock, inscr *HookInscription, mrc20Data *MRC20Protocol) (err error) {
	...
	if mrc20Data.Op == "transfer" {
		...
		// inscribe message
		jsonMessage := fmt.Sprintf(`{"tick":"%s","amount":"%s","hash":"%s"}`, mrc20Data.Tick, mrc20Data.Amt, block.BlockHash)
		b.RecorderPush(inscr.Address, "inscribe", jsonMessage)
		...
		//fmt.Println("@@ mrc20Data.Amt = ", amountBigInt.String())
		//fmt.Println("@@ balanceBigInt = ", balanceBigInt.String())
		...
		//fmt.Println("@@ newBalanceBigInt = ", newBalanceBigInt.String())
		...
		//fmt.Println("@@ totalBurnt = ", totalBurnt.String())
		...
		// burning message
		jsonMessage := fmt.Sprintf(`{"name":"%s","tick":"%s","amount":"%s" ,"hash":"%s"}`, mrc721Name, mrc20Data.Tick, amountBigInt.String(), block.BlockHash)
		b.RecorderPush(inscr.Address, "burning", jsonMessage)
