...
func (b *BTOrdIdx) GetMiningProfitChart(firstMrc721 *MRC721Protocol, step, max int) (string, error) {
	...
	for i := 0; i < max; i++ {
		...
		// Converts the summed value to a string format
		...
		if i%step == 0 || calcResult.IsMiningEnd {
			...
			formatFunds := func(funds string) string {
				...
				// First divide by 100000000 to round
				fundBigInt.Div(fundBigInt, big.NewInt(100000000))

				// Divide again by 100000000 and retain three decimals
				fundFloat := new(big.Float).SetInt(fundBigInt)
				fundFloat.Quo(fundFloat, big.NewFloat(100000000))

				// Formatted as a string with three decimal places
				return fmt.Sprintf("%.3f", fundFloat)
			}

			// Using the formatFunds function to process fund values
...
// InscriptionPlus contains the original HookInscription and a few additional fields
